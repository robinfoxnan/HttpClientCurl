...
namespace robin
{
	...
	public:
        ...
        double delta() const { return duration<double, span>(high_resolution_clock::now() - tpStart).count(); }

		template <typename span>
        double delta_restart() {
		...
		template <typename span>
        double stop_delta() { stop(); return duration<double, span>(tpStop - tpStart).count(); }

        double stop_delta_ms() { stop(); return duration<double, Timer::ms>(tpStop - tpStart).count(); }

		template <typename span>
        double stop_delta_start() {
			stop();
